...
class Config(dict):
    ...
    def postfix(self, *args):
        posts = []
        if '_grid_search_' in self:
            for k in sorted(self['_grid_search_'].keys()):
                v = self[k]
                if k in KEYWORD_MAP:
                    k = KEYWORD_MAP[k]
                if '/' in k:
    ...
    def get_or_default(self, key, default=None):
        if key in self.attr_cache:
            cache_res = self.attr_cache[key]
            if cache_res is None and default is not None:
                return default
        if key in self:
            res = self[key]
        else:
...
def load_specific_config(path, default_file_path="./config.yaml", git_update=True):
    if path.endswith("yaml") or path.endswith("yml"):
        ...
        with open(default_file_path, "r", encoding="utf-8") as cm, open(path, "r", encoding="utf-8") as sf:
